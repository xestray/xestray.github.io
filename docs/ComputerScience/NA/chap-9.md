...
假设 $A$ 是一个 $n \times n$ 的矩阵，且其特征值按绝对值大小排序为 $|\lambda_1| > |\lambda_2| \geqslant |\lambda_3| \geqslant ... \geqslant |\lambda_n| \geqslant 0$。这些特征值对应的 $n$ 各线性无关的特征向量 $\vec{v_1}, \vec{v_2}, ..., \vec{v_n}$，它们构成了 $R^n$ 的一个基。
...
$$ \vec{x}^{(k)} = \sum_{j=1}^{n} \beta_j \lambda_j^k \vec{v_j} = \lambda_1^k \sum_{j=2}^{n} \beta_j \left( \frac{\lambda_j}{\lambda_1} \right)^k \vec{v_j} $$

由于 $|\dfrac{\lambda_j}{\lambda_1}| < 1$ （$j = 2, 3, ..., n$），当 $k \to \infty$ 时，$\left( \frac{\lambda_j}{\lambda_1} \right)^k \to 0$，因此当 $k$ 足够大时，

$$ \begin{aligned}
\vec{x}^{(k)} &\approx \lambda_1^k \beta_1 \vec{v_1} \\
\vec{x}^{(k-1)} &\approx \lambda_1^{k-1} \beta_1 \vec{v_1} \\
\end{aligned} $$

我们就可以通过它们的比值求出主特征值 $\lambda_1$：
...
$$ \vec{u}^{(k-1)} = \frac{\vec{x}^{(k-1)}}{|\vec{x}^{(k-1)}_{p_{k-1}}|}, \quad \vec{x}^{(k)} = A\vec{u}^{(k-1)} $$
...
假设 $\lambda_1 > \lambda_2 \geqslant \lambda_3 \geqslant ... \geqslant \lambda_n$ 且 $|\lambda_2| > |\lambda_n| $，那么我们的目标就是让 $|\frac{\lambda_2}{\lambda_1}|$ 尽可能地小，这样收敛的速度就会更快
...
注意到 $|\frac{\lambda_2 - p}{\lambda_1 - p}| < |\frac{\lambda_2}{\lambda_1}|$，因此我们可以通过对矩阵 $B$ 使用幂法来加速收敛。
