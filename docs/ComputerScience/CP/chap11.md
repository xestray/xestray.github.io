...
在 chap10 的最末尾，我们计算出了**冲突图（interference graph）**，而在 chap11 中，我们的任务是为变量分配寄存器。一个很直观的想法是：在冲突图中相互连接的两个节点不能被分配到同一个寄存器中，因为它们的生命周期是重叠的。

我们可以将这个问题抽象为**图着色问题（graph coloring problem）**，也就是给冲突图中的每个节点分配一个颜色，使得相邻的节点不能有相同的颜色。如果颜色数量（可用的寄存器数量）不足以为所有节点分配颜色，那么我们就需要将一些变量溢出（spill）到内存中。

!!! abstract "寄存器分配整体流程"
    寄存器分配（或者说图着色问题）是一个 NP 完全问题，因此我们只能使用一种线性时间的近似算法来给出一个足够好的结果。我们学习的算法可以分为四个主要阶段：

    1. **Build**：根据活跃性构建冲突图
    2. **Simplify**：简化冲突图，反复移除度数小于 k 的节点，直到图为空或者所有节点的度数都大于等于 k，其中 k 是可用寄存器的数量。被移除的节点会被压入一个栈中，等待后续的着色阶段
    3. **Spill**：溢出处理，当图中所有节点的度数都大于等于 k 时，我们需要选择一个节点作为 potential spill variable，将其从图中移除，并将其标记为溢出变量。然后我们回到 Simplify 阶段，继续简化图
    4. **Select**：从栈中不断弹出节点，为其分配颜色（寄存器）；如果没有可用的颜色（邻居已用光 k 中颜色），则将其标记为溢出变量，发生 actual spill
...
!!! note "核心观察"
    ...
    --**充分性**：如果 $G'$ 可被 $k$ 着色，那么我们一定可以为节点 $m$ 分配一个不与其邻居冲突的颜色，从而得到 $G$ 的 $k$ 着色
...
Select 阶段是从栈中不断弹出节点，为其分配颜色（寄存器）。如果没有可用的颜色（邻居已用光 k 中颜色），则将其标记为溢出变量，发生 actual spill。

- 从一个空图开始，不断将节点从栈顶弹出，为其分配一个不与其邻居冲突的颜色（寄存器），尝试重建原始图
- 如果没有可用的颜色（邻居已用光 k 中颜色），则将其标记为溢出变量，发生 actual spill
...
!!! example
    ...
    这里的节点 b 是一个 potential spill 节点，但是我们其实可以为它分配一个不与任何邻居冲突的颜色（蓝色），在本次的图着色过程中并没有发生 actual spill。
...
如果在 select 阶段出现 actual spill，那么我们就需要重写整个程序，将发生 actual spill 的变量存储在内存中 —— 每次使用这个变量时都需要从内存中加载到寄存器中，每次修改这个变量时都需要将其从寄存器中写回到内存中。

- 在每一次 **use** 之前都插入一个 load 操作，将变量从内存中加载到寄存器中
- 在每一次 **def** 之后都插入一个 store 操作，将变量从寄存器中写回到内存中

这样一来，一个 spill 变量的生命周期就被分成了多个不连续的片段，相当于多个生命周期很短的 temporary，这些新变量是冲突图中的新节点，可能会与其他节点冲突。因此我们需要从头开始重新构建冲突图，并重新进行 build、simplify、select 阶段，直到没有 actual spill 发生为止。

- 实际中通常只需要 1~2 次迭代就可以收敛
...
在上一个 chap 构建干扰图时我们知道，如果两个变量在某个节点中被 MOVE 指令相互赋值，例如 `s := t`，那么它们存储的实际上是相同的值，可以不添加干扰边。一个直观的想法是，既然它们保存的是完全相同的值，那么实际上这个 MOVE 指令可以直接删去。
...
但是合并操作可能会导致一些问题：合并后的节点的度数可能会大于等于 K，从而导致原先是 k-可着色的图不再可以被 k 种颜色着色。为了避免这种情况，我们需要在合并前进行一些检查，确保合并后的节点仍然是 k-可着色的。有两种经典的方法来进行这种检查：**Briggs 方法**和 **George 方法**。

### Briggs

!!! note "Briggs 方法"
    节点 a 和 b 可以合并，当且仅当合并后的节点 ab 的高度数邻居（degree $\geqslant$ K）的数量少于 K 个。

**理由**：

- simplify 阶段会先移除所有的度数小于 K 的节点，因此此时 ab 只与留下来的高度数邻居相邻（其他邻居节点都被移除了）
...
1. **Build**：
    - 根据活跃性构建冲突图
    - 标记 move-related 节点（将 MOVE 指令的源和目标节点用虚线连接）
2. **Simplify**：
...
一部分寄存器被用于特殊用途，这些物理寄存器会与特殊的 temporary 永久绑定：
...
这些节点是 precolored（预着色）的，它们在冲突图中已经被分配了颜色（寄存器），并且有以下性质：
...
- 预着色节点可以与普通节点进行**保守合并**
    - 使用 George 判据是，必须选普通节点作为 a 来测试可合并性）

### Temporary Copies of Machine Registers

着色算法会不断进行 simplify、coalesce、spill 等操作，直到图中只剩下预着色节点为止。

因为预着色节点不会 spill，为了缩短它们的 live range，前端会生成相应的 MOVE 指令来将其中的值移出/移入到普通节点中，从而避免预着色节点的值在整个程序中都保持活跃，影响其他普通节点的着色。
...
- 如果寄存器的使用压力较小，t231 会和 r7 合并，因此 MOVE 指令会被消除，实际上值一直保存在 r7 中
...
- r3 是 callee-save 寄存器，因此需要一个临时变量 c 来保存它的值
...
我们可以发现没有 simplify 和 freeze 的机会，所有的非预着色节点的度数都大于等于 K，并且合并操作也是不行的。因此我们必须选择一个节点作为 potential spill 节点。
