...
解析（parsing）是编译器分析阶段的第二步，主要负责分析程序的短语结构，并为每个短语构建对应的抽象语法树（AST），属于**语法分析（Syntax Analysis）**。解析阶段的输入是词法分析器（lexer）输出的 token 流，输出则是一个抽象语法树。

- 一般而言，在进行语义分析时不会处理语法错误，语法错误会在解析阶段被捕获并报告，因此在语义分析阶段可以假设输入的程序已经是语法正确的。这种模块化的设计使得编译器的实现更加清晰和高效。

一个解析器通常会使用**上下文无关文法（Context-Free Grammar, CFG）**来描述编程语言的语法结构。解析器的主要任务是根据 CFG 来构建抽象语法树。

有两种解析方式：

- **Top-Down Parsing（自顶向下解析）**
    - 从语法树的根节点开始构建，逐步向下展开以匹配输入的 token 流。
    - 预测分析（Predictive Parsing）是一种常见的自顶向下解析方法，适用于 LL(1) 文法。
- **Bottom-Up Parsing（自底向上解析）**
    - 从输入的 token 流开始，从叶子节点逐步构建语法树，直到达到根节点。
...
因此我们需要更强大的工具来描述编程语言的语法结构，这就是**上下文无关文法（Context-Free Grammar, CFG）**。
...
我们把根据 CFG 生成字符串的过程称为**推导（Derivation）**，推导的结果可以用**语法树（Parse Tree）**来表示。推导的基本步骤如下：
...
- 一个字符串属于 $L(G)$ 当且仅当它可以从起始符号 $S$ 通过一系列产生式推导得到。
...
根据 CFG 来进行推导的过程可以用一个树状结构来直观地展示出来，这就是**语法树（Parse Tree）**。在语法树中：
...
在无二义性（Ambiguity）的 CFG 中，最左推导和最右推导会得到完全相同的语法树。
    - 这个例子是具有二义性的 CFG，因为它可以通过两种不同的推导方式生成相同的字符串 `id * id + id`，从而得到两棵不同的语法树。

### Ambiguous Grammarsv

如果存在一个字符串 $w$，使得它可以通过两种不同的推导方式生成，那么这个 CFG 就是有**二义性（Ambiguous）**的。二义性会导致解析器在构建语法树时产生歧义，从而无法确定正确的语法结构。
...
!!! question
    ...
    - 如何保证运算是左结合的？
        - 因为我们希望加减法和乘除法都是左结合的，即 `1 - 2 - 3` 应当被解析为 `(1 - 2) - 3` 而不是 `1 - (2 - 3)`。所以我们在定义 `E` 和 `T` 的产生式时，使用了递归的方式来表示左结合。例如在 `E` 的定义中，我们有 `E -> E + T | E - T | T`，其中左侧的 `E` 可以进一步地展开，进一步展开后的树节点会更加靠近叶子节点，会优先被计算，从而保证了运算的左结合性。
        - 简单来说，我们定义新的产生式时，要保证产生式右侧的第一个符号与左侧的符号相同，格式为 `X -> X op Y` 而非 `X -> Y op X`，其中 `op` 是一个运算符，这样就保证了运算的左结合性。

!!! tip
    有一些 language 仅存在有二义性的文法，而不存在无二义性的文法，这类 language 在编程语言的应用中可能存在问题

在语法分析时，我们需要确保整个文件都被完成地解析了，因此我们需要引入一个特殊的结束标记 $ \$ $（EOF），并且在文法中添加一个新的起始符号 $S'$ 和一条虚拟的产生式 $S' \to S \$ $。这样一来我们就可以表明在完成了 $S$ 的完整解析之后紧跟着的是 EOF 标记，从而确保整个文件都被完整地解析了。
...
!!! info
    - 人工实现的解析器通常使用 LL 文法，因为 LL 文法更容易理解和实现，适合于手工编写的解析器。
...
但对于某些文法来说，可能在接收到一个 token 时存在多种可能的产生式可以使用，这时我们就无法确定应该使用哪个产生式来展开语法树，这种情况就需要**回溯（Backtracking）**。
...
要在 $k=1$ 的情况下实现预测解析，我们需要明确知道每条产生式右侧可能出现的第一个**终结符**，这就需要我们构建**预测分析表（Predictive Parsing Table）**，以及构建预测分析标所需的三个概念：**Nullable**、**FIRST 集合**、**FOLLOW 集合**。

在以下的定义中，我们有如下假设：

- $\gamma,\ \beta,\ \delta$ 是任意的符号串（可能包含终结符和非终结符）
- $t$ 是一个终结符
- $X$ 是一个非终结符
...
!!! definition "FIRST 集合"
    ...
    > 假设 $w$ 是一个能从 $\gamma$ 推导出的字符串，并且它的第一个字符是一个终结符，我们把所有 $w$ 的第一个终结符 $t$ 收集起来，就构成了 $FIRST(\gamma)$ 集合。
    ...
    - 如果 $X$ 是一个非终结符，首先令 $FIRST(X) = \emptyset$，然后对于每个产生式 $X \to Y_1 Y_2 \cdots Y_k$，执行以下步骤：
        ...
        3. 如果 $Y_2$ 也是 Nullable 的，就重复上述步骤，直到遇到一个非 Nullable 的符号或者处理完所有的 $Y_i$。

!!! definition "FOLLOW 集合"
    $FOLLOW(X)$ 指的是在某个产生式中，可能紧跟在**非终结符** $X$ 的后面出现的终结符构成的集合。也就是说，如果存在一个产生式 $A \to \alpha X \beta$，那么 $FIRST(\beta) \subseteq FOLLOW(X)$，其中 $\alpha$ 和 $\beta$ 是任意的符号串。
...
有了 Nullable、FIRST 和 FOLLOW 集合之后，我们就可以构建预测分析表了。对于一个二维的预测分析表 $M$，每一行都对应一个非终结符 $X$，每一列对应下一个终结符 $t$（即 next-token，包括结束符号 $\$$）。
...
!!! note "LL(1) 文法"
    如果对于一个 CFG $G$，对于每个非终结符 $X$ 和每个终结符 $t$，预测分析表 $M[X, t]$ 中最多只有一条产生式，那么我们就称这个 CFG 是一个 LL(1) 文法。
    
    换句话说，LL(1) 文法是指在构建预测分析表时，每个单元格中最多只能有一条产生式，这样就保证了在解析过程中不会出现二义性，从而可以实现高效的预测解析。

!!! info "LL(k)"
    对于更一般的 LL(k) 文法，我们需要使用 $k$ 个 token 的向前查看来构建预测分析表。此时预测分析表的每一列都对应一个长度为 $k$ 的终结符序列，而不是单个终结符。
    
    当这个预测分析表中的每个单元格中最多只有一条产生式时，我们就称这个 CFG 是一个 LL(k) 文法。
...
**左递归（Left Recursion）**是指一个非终结符 $A$ 的产生式中直接或间接地包含了 $A$ 本身，并且 $A$ 是产生式右侧的第一个符号，例如 $A \to A \alpha$ 或 $A \to B \to A \beta$。

!!! example "左递归的例子"
    考虑一个文法：
    $$ E \to E + T \\ | \\ T $$
    其中 $E$ 是一个非终结符，$T$ 是另一个非终结符。这个文法是左递归的，因为$E$ 出现在产生式 $E \to E + T$ 右侧的第一个符号位置上。

    根据 $E \to E + T$ 这个产生式，我们可以知道 

    - $FIRST(E+T) \subset FIRST(E)$，因为 $FIRST(E)$ 包含 $FIRST(E+T)$ 中的所有终结符。
    - 又因为 $E$ 是 $E+T$ 的起始部分，就有 $FIRST(E) \subset FIRST(E+T)$，
    
    因此 $FIRST(E) = FIRST(E+T)$，这就导致了预测分析表中 $M[E, t]$ 中要么同时出现 $E \to E + T$ 和 $E \to T$ 两条产生式，要么就都没有这两条产生式，从而无法构建一个无冲突的 LL(1) 预测分析表。
...
LR(0) 的主要想法是通过构建一个状态机来表示解析过程中的各种状态，每个状态都对应于一个 LR(0) 项集（LR(0) Item Set）。LR(0) item 是一个产生式的右侧符号序列，其中有一个点号（dot）表示当前解析的位置。
...
我们可以定义各个 LR(0) item 之间的转移关系，从而得到一个 NFA。这个 NFA 的每一个状态都是对应于一个 item，每一个状态转移都对应于

- 接受一个 input token
    - 点号右侧是一个终结符或非终结符
    - 边的标签就是点号右侧的这个终结符或非终结符
    - 目标状态是把点号向右移动一位的 item
- 或者根据点号右侧的第一个非终结符进行移进
    - 点号右侧是一个非终结符
    - epsilon 转移（转移目标可能有很多个）
    - 目标状态是一个对应于这个非终结符的某个展开式的 item

通过对这个 NFA 进行子集构造，我们可以得到一个 DFA，这个 DFA 的每一个状态都对应于一个 LR(0) 项目集。

- 事实上 NFA 中每个状态的 $\varepsilon$-闭包就对应于 DFA 中的一个状态，可以利用这一特点进行快速的子集构造
...
!!! note "LR(0) Parsing 的算法"
    ...
    **Goto**：
    ...
    - 如果状态 $I$ 中包含 item 
        ...
        并且再对这个新的 item 进行 Closure 操作，得到一个新的状态 $J$，这样我们就定义了从状态 $I$ 到状态 $J$ 的转移。
...
- $x$ 是一个终结符，表示在当前状态下，当产生式右侧的符号串被完全处理后，期望接收到的下一个输入 token。
...
!!! note "LR(1) Parsing 的算法"
    ...
    **Goto**：
    ...
    - 与 LR(0) 的 Goto 几乎完全一致，只需要保留 lookahead 的部分即可。例如对于 item 
        ...
        $$ (A \to \alpha X \cdot \beta,\\ x) $$

        并且再对这个新的 item 进行 Closure 操作，得到一个新的状态。

    **Reduce**：

    - 当状态中存在一个完成了处理的 item $(A \to \alpha \cdot,\ x)$ 时，我们仅对于 $x$ 这个特定的终结符进行 reduce 操作，即仅在 $T[i, x]$ 中填入 `rk`，其中 $k$ 是产生式 $A \to \alpha$ 在文法中的编号（index）。
        - 这里的 $x$ 表示的是当前状态下的 lookahead 符号
...
LR(1) 解析得到的状态可能非常多，从而使得解析表也非常大。LALR(1) parsing 相当于对 LR(0) 和 LR(1) 的表达能力与表规模之间的一个折中方案。

LALR(1) 解析器的核心思想是将 LR(1) 中除了 lookahead symbol 以外完全相同的状态合并在一起，从而得到一个更小的状态集合。具体来说，我们首先构建 LR(1) 的 DFA，然后对于那些 LR(1) item 集合相同但 lookahead 不同的状态进行合并，得到一个新的 DFA，这个新的 DFA 就是 LALR(1) 的 DFA。

<figure markdown="span">
    ![](./assets/chap-3-36.png){width=70%}
</figure>

- 例如在上面这张图中，被红框圈出来的两个状态除了 lookahead 不同之外，它们的 LR(0) item 集合是完全相同的，因此我们可以将它们合并为一个新的状态，从而减少状态的数量。
- 黄框和绿框，可以把状态进行合并
...
!!! info "Burke-Fisher 错误修复机制"
    ...
    - 虽然这个方法有些暴力，但在很多情况下它确实能够成功地修复语法错误，从而使得 parser 能够继续解析后续的输入，提供更好的错误报告和用户体验。
