...
!!! tip
    这一章里很多东西都在 TCS 里学过了，关于 正则表达式、DFA、NFA 的内容只会简单写一写
...
2. **semantic value**：有一部分 token 还会有一个可选的语义值（semantic value），它是一个字符串，包含了 token 的具体文本内容
    ...
    - 而类型为 `COMMA` 的 token 则没有语义值，因为它的含义已经完全由 token name 表示了
...
!!! note "正则表达式运算的优先级"
    - `*` > `·` > `|`，即 Kleene Star 的优先级最高，连接操作的优先级次之，Alternation 的优先级最低
    - 除此之外还可以用括号 `()` 来改变运算的优先级，例如 `(a|b)*` 和 `a|b*` 是不同的正则表达式
...
!!! tip "tiger 语言中的注释" 
    ...
    - 这里被双引号 `"` 包裹的部分表示它本身的字面含义，例如 `"a+"` 就表示一个字母 `a` 后面跟着一个加号 `+`，而不是正则表达式的含义
...
- 各个状态之间的**转换（transition）**使用箭头表示，箭头上标注了触发转换的输入字符
    - 如果一个状态对于某个输入字符没有定义转换，那么就会被认为进入了一个**死状态（dead state）**，在这个状态下所有输入都会导致 error
    - 在 DFA 中，每一个状态对于每一个输入字符都有且只有一个定义的转换，不存在两条转换边具有相同的输入字符
...
!!! example "DFA Example"
    ...
    - 然后读入空格，状态 3 没有定义对于空格的转换，表示这个 token 结束了，此时我们就可以把刚刚读取到的 token 类型设为 `IF`，并且把输入指针回退到空格的位置，状态恢复到 0（因为我们已经完成了一个 token 的识别，所以要回到起始状态准备识别下一个 token）
    ...
    - 再次开始读入之后，我们依次读入了 `--not`，进入状态 10，但紧接着读入了一个横杠 `-`，状态 10 没有定义对于 `-` 的转换，这就表示 token 结束了，到那时此时处一个非终止状态，这是一个 error，我们可以选择忽略这个 token，继续从状态 0 开始读入下一个 token
...
- 对于正则表达式的 3 种基本构造方式，我们可以按照以下的方式来构建 NFA
    ...
    > 假设已经有两个正则表达式 $r$ 和 $s$，它们对应的 NFA $N_r$ 和 $N_s$ 也是已知的
...
!!! info "RE to NFA"
    ...
    - 波浪号包围的区域表示一个 NFA，伸到外边的线表示这个 NFA 的 start edge，NFA 中的圆圈表示 end state。
...
- 对于已经得到的每一个 DFA 状态（对应于 NFA 状态的子集 $A=\{a_1, a_2, \ldots, a_k\}$）
    - 对于每一个输入字符 $c$，从 $A$ 中的每个状态 $a_i$ 出发，计算在 NFA 根据输入字符 $c$ 和 $\varepsilon$-transition 可以到达的状态集合 $S_i$，然后将这些集合合并得到一个新的状态集合 $S = \bigcup_{i=1}^k S_i$，如果 $S$ 还没有被添加到 DFA 中，就将其添加为一个新的 DFA 状态，并且添加从状态 $A$ 到状态 $S$ 的转换，转换条件为输入字符 $c$
    - 如果 $S$ 中包含 NFA 的终止状态，那么 $S$ 也应该被标记为 DFA 的终止状态
...
!!! note
    ...
    - `closure` 函数的结果是从输入的状态集合出发，经过 $\varepsilon$-transition 可以到达的所有状态的构成的集合
    - `DFAedge(d, c)` 函数的结果是从状态集合 $d$ 出发，经过输入字符 $c$ 和 $\varepsilon$-transition 可以到达的所有状态的构成的集合
...
!!! info
    两个 DFA 状态 $p$ 和 $q$ 被认为是等价的，如果对于任意输入字符串 $w$，从状态 $p$ 出发经过输入字符串 $w$ 可以到达一个终止状态，当且仅当从状态 $q$ 出发经过输入字符串 $w$ 也可以到达一个终止状态。（即它们对于**任何**相同的输入字符串要么都接受，要么都拒绝）

    - 显然，两个等价状态要么都是终止状态，要么都不是终止状态，这可以作为判断状态是否等价的一个必要条件

我们说一个字符串 $w$ **区分（distinguishes）**两个状态 $p$ 和 $q$，如果从状态 $p$ 出发经过输入字符串 $w$ 可以到达一个终止状态，而从状态 $q$ 出发经过输入字符串 $w$ 不能到达一个终止状态，或者反过来。此时我们就称状态 $p$ 和 $q$ 是**可区分的（distinguishable）**的。

- 空字符串 $\varepsilon$ 可以区分一个终止状态和一个非终止状态
- 对于两个可区分的状态 $s$ 和 $t$，一定存在一个输入字符 $c$，使得 $s$ 到达状态 $s'$，$t$ 到达状态 $t'$，并且状态 $s'$ 和 $t'$ 也是可区分的
