...
我们称这些操作数为**地址**，地址可以是源代码中的**变量名**、**常量**、编译器生成的**临时变量**（例如 t1、t2 等）或
...
- 由于我们在这个布尔表达式里还不知道具体的 jump 目标地址，因此暂时将这些地址设置为 NULL，带等待后续的回填（back patching）阶段来填充这些地址
