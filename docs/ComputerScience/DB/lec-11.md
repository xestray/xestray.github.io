...
!!! definition "事务（transaction）"
    ...
    - **Isolation**: 隔离性，多个事务并发执行时，每个事务的执行不受其他事务的影响。
        - 也就是说，对于任意一对事务 $T_1$ 和 $T_2$，要么 $T_1$ 执行完毕之后 $T_2$ 才开始执行，要么 $T_2$ 执行完毕之后 $T_1$ 才开始执行。
...
这种方法要求对 db_pointer 的操作是原子的，即要么成功，要么失败；同时还要求不能有并发访问、磁盘不能 fail 等等。否则可能会导致数据不一致的情况发生。因此对于大型的数据库而言，shadow database scheme 并不适用。我们需要使用更复杂的恢复方法来实现事务的原子性和持久性。
...
!!! example
    ...
    我们把如上图所示的调度方式称为**串行调度（serial schedule）**，它在一个事务彻底执行完毕后才开始执行下一个事务。串行调度是最简单但效率底下啊的调度方式，它一定能保证数据库的一致性（本例中是保证 A+B=3000）。但是不同的串行调度方式未必是等价的（执行结果不同），例如上图的两种调度方式就不是等价的。
...
对于串行调度，我们有一个基本的假设：每个事务自己都能保证数据库的一致性，因此一组事务的串行执行也能保证数据库的一致性。我们把这个假设称为**可串行化（serializability）**，即一个调度是可串行化的，当且仅当它等价于某个串行调度。也就是说，如果一个调度是可串行化的，那么它一定能保证数据库的一致性。
...
- **冲突可串行化（conflict serializable）**: 
    - 基于操作之间的冲突关系定义
    - 如果两个操作之间没有冲突关系，那么它们可以交换位置，假如交换后可以把这个调度变成一个串行调度，那么我们就称这个调度是冲突可串行化的
...
对于来自事务 $T_i$ 和 $T_j$ 的两条指令 $I_i$ 和 $I_j$，我们说它们是**冲突的（conflicting）**，当且仅当：它们存在对于同一个数据项 $Q$ 的访问，并且至少有一个指令是写操作
...
直观上来说，如果两个操作之间存在冲突，那么它们间就会存在一个逻辑上的时间顺序。如果 $I_i$ 和 $I_j$ 在调度中是连续执行的，并且它们没有冲突，那么我们即使交换他们的位置也不会影响最终的结果
...
!!! definition "视图等价"
    假设调度 $S$ 和 $S'$ 中具有的事务相同，如果它们满足以下条件，我们就称 $S$ 和 $S'$ 是**视图等价（view equivalent）**的：
...
!!! example
    ...
    如上图所示，这一个调度不冲突等价与任何串行调度，但显然视图等价于调度 `<T27 T28 T29>`（交换前两个 write 操作）
...
!!! definiton "可恢复调度（recoverable schedule）"
    ...
    - 如果一个调度 $S$ 中 $T_i$ 要读取一个由 $T_j$ 写入的数据，那么 $T_j$ 的 commit 必须比 $T_i$ 的 commit 先执行。此时我们称这个调度是可恢复的
    - 如果一个调度 $S$ 中 $T_i$ 在 $T_j$ 之前提交，但 $T_j$ 没有读取到 $T_i$ 的写入结果，那么我们就称这个调度是不可恢复的
...
不难知道，一个调度是冲突可串行化的，当且仅当它的前驱图是无环的。因为如果有环存在的话，那么就说明有一些事务存在相互依赖的关系，这些事务不可串行化。

!!! note
    - 通常前驱图仅用于数学原理上的分析，在实际应用中不会等到检测完毕这个调度是否冲突可串行化之后才开始执行
        - 也就是说不会等到分析完毕之后才开始执行事务
    - 前驱图并不能用于检查视图可串行化，检查调度是否视图可串行化是一个 NP 完全问题
        - 但我们可以通过其他的使用算法来寻找试图可串行化的充分条件
