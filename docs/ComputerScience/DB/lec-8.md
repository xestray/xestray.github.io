...
!!! definition "B+ 树"
    ...
    - 每个不是根或叶的节点都有 $\ceil (\frac{m}{2})$ 到 $m$ 个子节点（$m$ 为节点的阶数）
    - 叶节点有 $\ceil (\frac{m-1}{2})$ 到 $m-1$ 个键值
...
!!! property "叶子节点"
    ...
    - 每个叶子节点所具有的搜索键数量必须在 $\ceil (\frac{m-1}{2})$ 到 $m-1$ 之间。
        - 例如 $m=5$ 时，叶子节点中搜索键的数量可以是 2, 3 或 4。
    - $P_n$ 会指向按 search-key 排序下一个叶子节点，这便于我们对文件的顺序处理。

!!! property "非叶子节点"
    - 指针（子树）的数量在 $\ceil (\frac{m}{2})$ 到 $m$ 之间。我们定义扇出数（fanout）为节点中指针的数量。
