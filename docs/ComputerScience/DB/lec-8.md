...
- Hash indices(哈希索引): search keys (index entries) are distributed uniformly across "buckets" using a "hash function"
...
稠密索引（dense index）：文件中每一个搜索键的值都有一个索引项。
...
- 即使使用二分查找，我们也需要 $\log_2(1000) = 9$ 次 I/O 操作才能找到目标索引项。
...
即使外层索引仍然过大，我们也可以再给它建立一个稀疏索引。同理可以推导到更多层的索引，这样就建立了一个多级索引（multilevel index）。
...
- 否则，（多条记录，即 search-key 无唯一性
    - （使用辅助索引）如果有指向所有带有相同搜索键值得记录的指针，那么就从索引项中删除指向被删除的记录的指针
    - （使用主索引）否则，如果被删除的的记录是指针指向的第一条记录，就把指针指向下一条记录
        - 否则就不需要对索引进行任何修改
...
对于多级索引，我们可以由底层逐级向上层扩展，每一层的处理过程与上述单层索引情况下类似。
...
- 如果新插入的记录的搜索键值在索引中不存在，则插入一个包含该搜索键值和指向新记录的指针的索引项
- 否则，
    - 如果是使用多条指针，那么就在索引项中新增一条指向新记录的指针
    - 否则，就不需要对索引项做任何操作
...
多级索引的插入操作相当于在单级索引的基础上继续进行拓展：从底层向上层扩展，每一层的处理过程与上述单层索引情况下类似。
...
!!! definition "B+ 树"
    ...
    - 每个不是根或叶的节点都有 $\lceil \dfrac{m}{2} \rceil$ 到 $m$ 个子节点（$m$ 为节点的阶数）
    - 叶节点有 $\lceil \dfrac{m-1}{2} \rceil$ 到 $m-1$ 个键值

    特殊情况：

    - 如果根节点不是叶节点，那么它至少有两个子节点
    - 如果根节点是叶节点（即树中只有一个节点），那么它有 $0$ 到 $m-1$ 个键值
...
!!! property "叶子节点"
    - 对于 $i = 1, 2, ..., n-1$，$P_i$ 要么指向搜索键值为 $K_i$ 的记录，要么指向一个装有指针的 bucket，里面每个指针所指向的记录都有相同的搜索键值 $K_i$。
    ...
    - 每个叶子节点所具有的搜索键数量必须在 $\lceil \dfrac{m-1}{2} \rceil$ 到 $m-1$ 之间。
        - 例如 $m=5$ 时，叶子节点中搜索键的数量可以是 2, 3 或 4。
    - $P_n$ 会指向按 search-key 排序下一个叶子节点，这便于我们对文件的顺序处理。

!!! property "非叶子节点"
    - 指针（子树）的数量在 $\lceil \dfrac{m}{2} \rceil$ 到 $m$ 之间。我们定义扇出数（fanout）为节点中指针的数量。
    - 非叶子结点在叶子节点上形成了一个多级的稀疏索引，对于拥有 $n$ 指针的非叶子结点：
        - $P_1$ 所指的子树中的所有 search-keys 皆小于 $K_1$
        - 对于 $2 \leqslant i \leqslant n-1$，$P_i$ 所指的子树中的所有 search-keys 皆大于等于 $K_{i-1}$ 且小于 $K_i$ 之间（即 $K_{i-1} \leqslant K < K_i$）
...
- 对主文件的插入和删除可以高效地处理，对索引的重构可以在对数时间内完成
...
- 从根节点开始，在每一层中找到第一个大于等于 $V$ 的搜索键值
    ...
    - 否则进入最后一个非空指针所指向的子节点
- 这么逐层向下寻找直到到达叶子节点
    - 如果在叶子节点中找到了与 $V$ 相等的搜索键值，返回 $i$ 最小的 $
    - 否则就说明没有搜索键值与 $V$ 相同的记录

对于有重复的搜索键值

- 在叶节点和中间节点中，我们不能保证 $K_1 < K_2 < ... < K_{n-1}$，但我们可以保证 $K_1 \leqslant K_2 \leqslant ... \leqslant K_{n-1}$。
- $P_i$ 指向的子树中的 search-key 值都 $\leqslant K_i$，但未必 $< K_i$。
- 我们需要对上述的查询算法进行一些修改：
    - 即使当 $V = K_i$ 时，我们也需要继续遍历 $P_i$ 所指向的子树
    - 当我们到达一个叶子节点 $C$ 时，我们要检查 $C$ 中的搜索键值是否都小于 $V$。如果是，那就在检查 $C$ 中是否包含 $V$ 之前令 $C$ = 它的右兄弟

!!! note 
    - 如果文件中有 $K$ 个搜索键值，那么 B+ 树的高度不会超过 $\lceil \log_{\lceil n/2 \rceil} (K) \rceil$。
        - 例如有一百万个搜索键值，且 $n=100$，那么树就有 $\lceil \log_{50} (1000000) \rceil = 4$ 层，即我们只需要 4 次访问磁盘的操作（访问 4 次节点）。
...
2. 检查这个搜索键值是否已经存在
    ...
    - 如果不存在
        ...
        - 如果叶子节点中还有空间，就把 {搜索键值，指针} 对插入到叶子节点中
        - 否则就在插入新条目后把叶子节点分裂
3. 分裂叶子节点
    - 将叶子节点中的 $n$ 个{搜索键值，指针}对（包括新插入的对）按照顺序排列
    - 把前 $\lceil n/2 \rceil$ 个 {搜索键值，指针} 对放在原来的叶子节点中，剩下的放在新的叶子节点中
    - 假设新的节点为 p，k 为 p 中最小的键值，在被分裂的节点的父节点中插入（k，p）
    - 如果父节点已满，那么再把父节点分裂，向上传播
4. 向上传播
...
4. 重新分配指针
    - 如果由于删除而导致节点的 entry 数量过少，但该节点与它兄弟节点的 entry 不能合并为一个节点，那么对指针的内容进行重新分配：
        ...
        - 更新父结点中相应的搜索键值
5. 级联删除
    - 节点的删除操作可能需要向上级联传播，直到遇见一个有 $\lceil n/2 \rceil$ 个或更多个指针的节点为止
6. 删除根节点
    - 如果在删除后根节点只有一个指针，那么就把原来的根节点删除，唯一的子节点成为新的根节点，树的高度减少 1
...
??? example "Example of B+ Tree Insertion"
    ...
    从最后两张图中我们可以看到，当我们把 "Singh" 和 "Wu" 都删除之后，"Kim" 就会转移到右边的叶子节点中，相当于最右边的叶子节点从它的左兄弟那里借了一个值过来。并且他们的父节点的搜索键值也会进行相应的更新。
...
- 严格来说，哈希索引总是作为辅助索引
    - 如果文件本身就是通过哈希组织起来的，那么与之独立的使用相同搜索键的主哈希索引就是没有必要的。
    - 但通常来说，我们会使用术语“哈希索引”来代指辅助索引结构和哈希文件结构。
- 哈希索引也有自己的劣势
...
- 我们总是只使用这个数值的前 $i$ 位来作为桶的地址，$0 \leqslant i \leqslant 32$。$i$ 是一个动态变化的值，随着数据量的增加而增加。
- 桶地址表的大小为 $2^i$，最开始时 $i=0$，只有一个桶。
- 当数据库增大时，$i$ 会随之增大，从而可以容纳更多的条目。
    - 由于多个 entry 可能处于同一个桶中，因此实际使用到的桶的数量会小于 $2^i$。
- 每个桶 $j$ 都对应一个值 $i_j$，桶里所有 entry 对应的哈希值的前 $i_j$ 位都与桶 $j$ 的地址相同。

<figure markdown="span">
    ![](./assets/索引16.png){width=60%}
</figure>

如果我们想要定位一个搜索键值为 $K_j$ 的记录：

1. 计算哈希值 $h(K_j) = X$
2. 使用 $X$ 的前 $i$ 位作为桶地址表的索引，根据表中的指针找到对应的桶

如果我们想要一个**搜索**键值为 $K_j$ 的记录

1. 按上面的步骤找到对应的桶 $j$
2. 如果桶 $j$ 中有足够空间，就把记录插入桶中
3. 如果空间不足，就必须分裂桶并重新尝试插入
    - 在某些情况下我们也可以采用溢出桶

如果插入搜索键值为 $K_j$ 的记录时，我们要分裂桶 $j$。

- 如果 $i > i_j$（多个指针指向 $j$）
    - 分配一个新的桶 $z$，并且把 $i_z$ 设置为 $i_j + 1$
    - 把桶地址表中指向 $j$ 的后半部分 entry 改为指向 $z$
    - 把桶 $j$ 中的所有记录移除，把它们重新插入
    - 此时再重新计算记录 $K_j$ 对应的桶并插入（如果此时新对应的桶还是满的，就需要再次分裂）
- 如果 $i = i_j$（只有一个指针指向 $j$）
    - $i$ 递增 1，并把桶地址表的大小翻倍
    - 把表中的每一个 entry 替换为两个指向同一个桶的 entry
    - 重新计算 $K_j$ 的桶地址表条目，现在就变为了 $i > i_j$ 的情况
- 如果在插入一个值时，$i$ 已经达到了最大值 $b$，那么就可以开始创建溢出桶而非进一步扩大桶地址表的大小
...
- 也可以把桶进行合并
    - 仅当两个桶的 $i_j$ 值相同，并且前 $i_j - 1$ 位相同时才可以合并
- 也可以把桶地址表的大小缩小
...
??? example
    ...
    我们使用 branch-name 来计算哈希值，桶的容量为 2。最开始时 $i=0$，只有一个桶。
    ...
    再插入一条 Mianus 记录之后，由于 Downtown 和 Mianus 的哈希值的最高 1 位都是 1，因此 Downtown 对应的桶需要分裂，而此时会有两个桶地址表条目都指向 Brighton 所在的桶

    <figure markdown="span">
        ![](./assets/索引20.png){width=65%}
    </figure>

    再插入三个 Perryridge 之后，由于 $i$ 达到了最大限制，此时我们就要开始使用溢出桶。
    
    这时候就有四个条目指向 Brighton，两个条目指向 Downtown，一个条目指向 Mianus，一个条目指向 Perryridge。

    <figure markdown="span">
        ![](./assets/索引21.png){width=65%}
    </figure>

    最后我们再插入 Redwood 和 Round Hill，由于这两次插入都不需要分裂桶，因此桶地址表实际上不会发生任何改变。

!!! info "Comparison of Ordered Indexing and Hashing"
    ...
    - 权衡平均访问时间和最坏访问时间
    - 预期的查询类型
        - 在使用明确的键值进行查询时，哈希索引会更高效
        - 但如果我们常需要范围查询，那么有序索引会更高效

## Write-optimized Indexs 

### LSM Tree

LSM 树（Log-Structured Merge Tree）是一种用于高效地处理写密集型数据存储的树形数据结构，特别适用于需要高频率插入和较少查询的场景。LSM Tree 旨在优化磁盘的写操作性能，尤其是在写入操作频繁的场景中。传统的 B 树在执行写入操作时，可能会频繁发生磁盘 I/O 操作，这会影响性能。而 LSM Tree 通过将多个写操作合并后批量写入磁盘，从而大大减少了磁盘的 I/O 操作，提高了写入性能。

- LSM 树的写入会首先在内存中，数据会被写入到内存中的一个数据结构中（通常是一个平衡树或跳表），称为 MemTable。
- 当 MemTable 达到一定大小时，它会被转化为一个不可变的文件结构，通常是 SSTable（Sorted String Table），然后写入到磁盘中
- 随着磁盘中的 SSTable 数量的增加，LSM Tree 会定期（或同样大小的 SSTable 数量到达一个阈值时）将多个 SSTable 合并成一个更大的 SSTable，这个过程称为合并（Merge）。
...
!!! example
    ...
    - 当 $L_1$ 树超过某个阈值时，它们就会被合并称为 $L_2$ 树，以此类推得到更多的层级
        - $L_{i+1}$ 树的阈值总是 $L_i$ 树的 $k$ 倍 
...
!!! note "Deletion of LSM Tree"
    - 删除操作会在内存中的 MemTable 中进行，删除的记录会被标记为删除
    - 查找操作会同时找到原始记录和删除记录，并且只返回不与删除条目匹配的原始记录
    - 当树需要合并时，如果有原始条目和删除记录匹配，那么两者都会被去除而不会参与合并
    - 更新操作通过插入+删除的方式来实现：新的记录会被插入到 MemTable 中，而旧的记录会被标记为删除

### Buffer Tree

Buffer Tree 是 LSM Tree 的替代方案，它是 B+ 树的一种变体。Buffer Tree 的每一个 B+ 树节点都是一个缓冲区，当缓冲区满时，会将其移动到更低的层级中。
...
我们也可以通过创建唯一索引来间接地指定和强制执行搜索键是候选键的条件：
